import { h, H as Host, r as registerInstance } from "./index-C5zPPjub.js";
var messageBallonSvg = "data:image/svg+xml;base64,";
var loadingPageCss = ":host{display:block;position:fixed;top:0;left:0;height:100vh;width:100vw;z-index:999}.loading-container{display:-ms-flexbox;display:flex;-ms-flex-align:center;align-items:center;-ms-flex-pack:center;justify-content:center;width:100%;height:100%;background-color:var(--color-surface-4, rgb(20, 20, 20));opacity:0.65;mix-blend-mode:multiply}.page_loading{-webkit-animation:growUp 0.8s ease-in-out infinite;animation:growUp 0.8s ease-in-out infinite;width:116px;height:128px;color:var(--color-surface-1, rgb(246, 246, 246))}@-webkit-keyframes growUp{from{opacity:0}10%{opacity:1}60%{opacity:1}to{-webkit-transform:scale(1.6);transform:scale(1.6);opacity:0}}@keyframes growUp{from{opacity:0}10%{opacity:1}60%{opacity:1}to{-webkit-transform:scale(1.6);transform:scale(1.6);opacity:0}}";
var BdsLoading = function() {
  function M(M2) {
    var D = this;
    registerInstance(this, M2);
    this.dataTest = null;
    this.formatSvg = function(M3) {
      var D2 = document.createElement("div");
      D2.innerHTML = M3;
      var N = D2.firstElementChild;
      N.removeAttribute("width");
      N.removeAttribute("height");
      return D2.innerHTML;
    };
    this.setSvgContent = function() {
      var M3 = messageBallonSvg;
      var N = atob(M3.replace("data:image/svg+xml;base64,", ""));
      D.svgContent = D.formatSvg(N);
    };
  }
  M.prototype.componentWillLoad = function() {
    this.setSvgContent();
  };
  M.prototype.render = function() {
    return h(Host, { key: "01c10373b36658a99c810fe6dcf7b9a40698f636" }, h("div", { key: "8fbf9a177462daaecec6f92f318d20a1a9906c3d", class: "loading-container", "data-test": this.dataTest }, h("div", { key: "48a7facd88861f573b09f408b1623408cd48300d", class: { page_loading: true }, innerHTML: this.svgContent })));
  };
  return M;
}();
BdsLoading.style = loadingPageCss;
export {
  BdsLoading as bds_loading_page
};
//# sourceMappingURL=bds-loading-page.entry-Ds6lqFPd.js.map
