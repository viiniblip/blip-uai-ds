import { h, r as registerInstance, c as createEvent } from "./index-C5zPPjub.js";
var __awaiter = function(t, o, e, i) {
  function a(t2) {
    return t2 instanceof e ? t2 : new e(function(o2) {
      o2(t2);
    });
  }
  return new (e || (e = Promise))(function(e2, n) {
    function s(t2) {
      try {
        d(i.next(t2));
      } catch (t3) {
        n(t3);
      }
    }
    function r(t2) {
      try {
        d(i["throw"](t2));
      } catch (t3) {
        n(t3);
      }
    }
    function d(t2) {
      t2.done ? e2(t2.value) : a(t2.value).then(s, r);
    }
    d((i = i.apply(t, o || [])).next());
  });
};
var __generator = function(t, o) {
  var e = { label: 0, sent: function() {
    if (n[0] & 1) throw n[1];
    return n[1];
  }, trys: [], ops: [] }, i, a, n, s;
  return s = { next: r(0), throw: r(1), return: r(2) }, typeof Symbol === "function" && (s[Symbol.iterator] = function() {
    return this;
  }), s;
  function r(t2) {
    return function(o2) {
      return d([t2, o2]);
    };
  }
  function d(r2) {
    if (i) throw new TypeError("Generator is already executing.");
    while (s && (s = 0, r2[0] && (e = 0)), e) try {
      if (i = 1, a && (n = r2[0] & 2 ? a["return"] : r2[0] ? a["throw"] || ((n = a["return"]) && n.call(a), 0) : a.next) && !(n = n.call(a, r2[1])).done) return n;
      if (a = 0, n) r2 = [r2[0] & 2, n.value];
      switch (r2[0]) {
        case 0:
        case 1:
          n = r2;
          break;
        case 4:
          e.label++;
          return { value: r2[1], done: false };
        case 5:
          e.label++;
          a = r2[1];
          r2 = [0];
          continue;
        case 7:
          r2 = e.ops.pop();
          e.trys.pop();
          continue;
        default:
          if (!(n = e.trys, n = n.length > 0 && n[n.length - 1]) && (r2[0] === 6 || r2[0] === 2)) {
            e = 0;
            continue;
          }
          if (r2[0] === 3 && (!n || r2[1] > n[0] && r2[1] < n[3])) {
            e.label = r2[1];
            break;
          }
          if (r2[0] === 6 && e.label < n[1]) {
            e.label = n[1];
            n = r2;
            break;
          }
          if (n && e.label < n[2]) {
            e.label = n[2];
            e.ops.push(r2);
            break;
          }
          if (n[2]) e.ops.pop();
          e.trys.pop();
          continue;
      }
      r2 = o.call(t, e);
    } catch (t2) {
      r2 = [6, t2];
      a = 0;
    } finally {
      i = n = 0;
    }
    if (r2[0] & 5) throw r2[1];
    return { value: r2[0] ? r2[1] : void 0, done: true };
  }
};
var modalCss = '.modal__dialog{opacity:0;visibility:hidden;width:100%;height:100%;position:fixed;top:0;left:0;-webkit-transition:opacity 0.3s ease-in-out;transition:opacity 0.3s ease-in-out;z-index:80000;display:none}.modal__dialog .outzone{position:absolute;inset:0;background-color:var(--color-content-din, rgb(0, 0, 0));opacity:0.7}.modal__dialog--dynamic{overflow-y:auto;padding-top:40px;padding-bottom:40px;height:-webkit-fill-available}.modal__dialog .modal{position:relative;margin:auto;width:592px;height:368px;border-radius:8px;background:var(--color-surface-1, rgb(246, 246, 246));-webkit-box-shadow:0px 8px 4px -4px var(--color-shadow-0, rgba(0, 0, 0, 0.04)), 0px 12px 12px -4px var(--color-shadow-1, rgba(0, 0, 0, 0.16));box-shadow:0px 8px 4px -4px var(--color-shadow-0, rgba(0, 0, 0, 0.04)), 0px 12px 12px -4px var(--color-shadow-1, rgba(0, 0, 0, 0.16));padding:32px;display:-ms-flexbox;display:flex;-ms-flex-direction:column;flex-direction:column;-ms-flex-pack:justify;justify-content:space-between}.modal__dialog .modal--dynamic{height:auto;width:auto;max-width:1000px}.modal__dialog .modal .close-button{position:relative;color:var(--color-content-default, rgb(40, 40, 40));-ms-flex-item-align:end;align-self:flex-end;margin-bottom:16px;cursor:pointer}.modal__dialog .modal .close-button::before{content:"";position:absolute;inset:-4px;border:2px solid transparent;border-radius:4px}.modal__dialog .modal .close-button:focus-visible{outline:none}.modal__dialog .modal .close-button:focus-visible::before{border-color:var(--color-focus, rgb(194, 38, 251))}.modal__dialog .modal .slot--dynamic{-ms-flex:1 1 auto;flex:1 1 auto}.modal__dialog--open{opacity:1;visibility:visible;display:-ms-flexbox;display:flex}';
var BdsModal = function() {
  function t(t2) {
    var o = this;
    registerInstance(this, t2);
    this.bdsModalChanged = createEvent(this, "bdsModalChanged");
    this.open = false;
    this.closeButton = true;
    this.size = "fixed";
    this.outzoneClose = true;
    this.enterClose = true;
    this.dtOutzone = null;
    this.dtButtonClose = null;
    this.listener = function(t3) {
      if (o.enterClose && (t3.key == "Enter" || t3.key == "Escape")) {
        o.toggle();
      }
    };
    this.handleMouseClick = function() {
      o.open = false;
      o.bdsModalChanged.emit({ modalStatus: "closed" });
    };
    this.onClickCloseButtom = function() {
      if (o.outzoneClose === true) {
        o.open = false;
        o.bdsModalChanged.emit({ modalStatus: "closed" });
      }
    };
  }
  t.prototype.toggle = function() {
    return __awaiter(this, void 0, void 0, function() {
      return __generator(this, function(t2) {
        this.open = !this.open;
        if (this.open) {
          this.bdsModalChanged.emit({ modalStatus: "opened" });
        } else {
          this.bdsModalChanged.emit({ modalStatus: "closed" });
        }
        return [2];
      });
    });
  };
  t.prototype.isOpenChanged = function() {
    if (this.open) {
      document.addEventListener("keydown", this.listener, false);
    } else document.removeEventListener("keydown", this.listener, false);
  };
  t.prototype.render = function() {
    var t2, o, e;
    var i = this;
    return h("div", { key: "7c2245485dc05607d6bf3abc4ba765dcc1a56c26", class: (t2 = { modal__dialog: true, "modal__dialog--open": this.open }, t2["modal__dialog--".concat(this.size)] = true, t2) }, h("div", { key: "5129cec6e970ab5688100a916e55b28f615208ac", class: { outzone: true }, onClick: function() {
      return i.onClickCloseButtom();
    }, "data-test": this.dtOutzone }), h("div", { key: "3ae0b0b3cdad6485c8cf435ed6fbf053435a8b82", class: (o = { modal: true }, o["modal--".concat(this.size)] = true, o) }, this.closeButton && h("bds-icon", { key: "a8907580ef453f5227559bbcdd5af19f5c82e2b2", size: "medium", class: "close-button", name: "close", tabindex: "0", onClick: this.handleMouseClick, dataTest: this.dtButtonClose }), this.size == "fixed" && h("slot", { key: "96c3a2963b970ddd6a98c2905e6d5c5c58463828" }), this.size !== "fixed" && h("div", { key: "61969dce2b8f71df5295ebd738fab61c9d50d367", class: (e = { slot: true }, e["slot--".concat(this.size)] = true, e) }, h("slot", { key: "37d83792edd16c2ce7a7807dfcb722375fd98119" }))));
  };
  Object.defineProperty(t, "watchers", { get: function() {
    return { open: ["isOpenChanged"] };
  }, enumerable: false, configurable: true });
  return t;
}();
BdsModal.style = modalCss;
export {
  BdsModal as bds_modal
};
//# sourceMappingURL=bds-modal.entry-DG-vliPR.js.map
